...
#    'model': 'cfg/yolo.cfg',            #settingan YOLO bawaan
#    'load': 'bin/yolo.weights',         #weight YOLO bawaan
    'threshold': 0.15,
    'gpu': 0.5
}
...
channel.queue_declare(queue='servertoholo', durable=False, arguments={'x-max-length':2})

channel.queue_declare(queue='notif', durable=True, arguments={'x-max-length':2})

channel.exchange_declare(exchange='ex_servertoholo', exchange_type='topic', durable=False)
...
#ketika ada notifikasi di rmq maka fungsi callback akan jalan
def callback(ch, method, properties, body):
	...
	if body[0] == 49:
		...
		if (sum_box == 0) :
			...
			#apabila tidak ada box terdeteksi akan diberi nilai 0 untuk semua var
			data = []
			...
			#apabila ada box terdeteksi seluruh nilai var akan diberikan
			data = []
