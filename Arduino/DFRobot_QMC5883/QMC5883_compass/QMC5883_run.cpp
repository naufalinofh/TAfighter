...
Vector QMC5883_run::readNormalize(void)
{
  int range = 10;
  float Xsum = 0.0;
  float Ysum = 0.0;
...
uint8_t QMC5883_run::fastRegister8(uint8_t reg)
{
  uint8_t value=0;
  Wire.beginTransmission(QMC5883_ADDRESS);
    #if ARDUINO >= 100
...
int16_t QMC5883_run::readRegister16(uint8_t reg)
{
  int16_t value=0;
    Wire.beginTransmission(QMC5883_ADDRESS);
    #if ARDUINO >= 100
